...
-- after:
SELECT "dim_users"."name" AS "name", SUBSTRING(CAST("dim_users"."email" AS TEXT), 1, 1) || '***@' || SPLIT_PART(CAST("dim_users"."email" AS TEXT), '@', 2) AS "email", '***-' || RIGHT(CAST("dim_users"."phone" AS TEXT), 4) AS "phone" FROM "dim_users" AS "dim_users" LIMIT 5;
...
-- after:
SELECT "u"."name" AS "name", SUBSTRING(CAST("u"."email" AS TEXT), 1, 1) || '***@' || SPLIT_PART(CAST("u"."email" AS TEXT), '@', 2) AS "email", NULL AS "ssn", "o"."total" AS "total" FROM dim_users AS "u" JOIN "orders" AS "o" ON "o"."user_id" = "u"."id" ORDER BY "o"."total" DESC LIMIT 10;
