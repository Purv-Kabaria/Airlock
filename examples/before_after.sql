...
-- after:
SELECT "dim_users"."name" AS "name", SUBSTRING(CAST("dim_users"."email" AS TEXT), 1, 1) || '***@' || SUBSTRING(CAST("dim_users"."email" AS TEXT), STRPOS(CAST("dim_users"."email" AS TEXT), '@') + 1) AS "email", CASE WHEN LENGTH(CAST("dim_users"."phone" AS TEXT)) >= 7 THEN '***-' || RIGHT(CAST("dim_users"."phone" AS TEXT), 4) ELSE '***' END AS "phone" FROM "dim_users" AS "dim_users" LIMIT 5;
...
-- after:
SELECT "u"."name" AS "name", SUBSTRING(CAST("u"."email" AS TEXT), 1, 1) || '***@' || SUBSTRING(CAST("u"."email" AS TEXT), STRPOS(CAST("u"."email" AS TEXT), '@') + 1) AS "email", NULL AS "ssn", "o"."total" AS "total" FROM dim_users AS "u" JOIN "orders" AS "o" ON "o"."user_id" = "u"."id" ORDER BY "o"."total" DESC LIMIT 10;
...
-- after:
WITH "ranked" AS (SELECT "dim_users"."name" AS "name", SUBSTRING(CAST("dim_users"."email" AS TEXT), 1, 1) || '***@' || SUBSTRING(CAST("dim_users"."email" AS TEXT), STRPOS(CAST("dim_users"."email" AS TEXT), '@') + 1) AS "email", ROW_NUMBER() OVER (ORDER BY "dim_users"."signup_date" DESC) AS "rn" FROM "dim_users" AS "dim_users") SELECT "ranked"."name" AS "name", "ranked"."email" AS "email" FROM "ranked" AS "ranked" WHERE "ranked"."rn" <= 3 LIMIT 10000;
