...
Needs a model that follows a multi-step tool workflow. Small local models (3B and under) tend to
guess column names instead of calling warehouse_describe_table first, which produces a real but
unflattering transcript. Anything in the 7B-and-up class, or a hosted model, behaves.
...
class Transcript:
    ...
    def save(self, path: Path, provider: Provider) -> None:
        header = (
            ...
            "over stdio with no warehouse credential of its own. Whatever the model does here is "
            "what it did - the header does not assert that it recovered, because a capture that "
            "claims an outcome the transcript does not show is worse than no capture. "
