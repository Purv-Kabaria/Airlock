...
- **3-minute demo path:** `python demo/up.py` (any OS) → open the included MCP client config → run the three scripted prompts in `demo/RECORDING.md` (clean · PII · deprecated-table) → `airlock tail` in a second pane shows live verdicts → the DataHub UI shows the write-back ledger.
...
- **Sample outputs, no setup:** [`examples/`](examples/) has captured request/response envelopes, before/after SQL pairs, and a full audit log — regenerated by `make examples`, never hand-edited. [`examples/agent_session.md`](examples/agent_session.md) is the one to read if you only read one: a real MCP session where an agent asks for a customer by SSN, is refused with a reason it can act on, reads each column's policy off `warehouse_describe_table`, and reformulates into a query that answers what the user actually wanted. [`examples/reformulation.md`](examples/reformulation.md) is the same loop in three shorter chains. That loop is the thing no other tool in this space does.
...
- **Written for upstream, published here:** two pieces in [`contrib/`](contrib/), kept dependency-isolated so either drops into its target project unchanged. A push-based snapshot-refresh [DataHub Action](contrib/datahub_action/), and [`datahub-audit`](contrib/datahub_audit_skill/), a skill the `datahub-skills` registry already routes users to but which was never written. Both are Apache 2.0 in this repo and ready to file; neither has been submitted as a pull request yet, so take them as prepared work rather than merged contributions.
