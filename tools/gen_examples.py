...
def _readme() -> str:
    ...
    return (
        ...
        "`agent_session.md` is a captured MCP session: an agent asks for a customer by SSN, is "
        "refused with a reason it can act on, reads the policy off `warehouse_describe_table`, "
        "and reformulates into a query that answers the underlying request. Every envelope in "
        "it is real; the agent turns were taken by a model reading each response.\n\n"
        "`agent_reformulation.md` is generated on demand rather than committed: "
        "`python demo/agent_reformulation.py --capture` drives a real LLM through these same tools "
        "over MCP, and what it contains depends entirely on the model you point at it. It runs on "
        "Anthropic or any OpenAI-compatible endpoint (OpenAI, Together, Groq, a local Ollama or "
        "vLLM), so it is not tied to one vendor - but a model that cannot follow a multi-step tool "
        "workflow produces a transcript of it guessing column names, which is a real run and poor "
        "evidence. `reformulation.md` above covers the same loop without that dependency.\n"
    )
